import { AnchorElement, ButtonElement, CardPanel, Component, DivElement, H4Element, TextField, VBox } from "typecomposer";
...
export class ForgotPage extends Component {
	...
	constructor() {
		...
		const card = new CardPanel({ width: "400px" });
		const vbox = new VBox({ padding: "10px", gap: "15px" });

		vbox.append(new H4Element({ text: "Matcha", className: "login_header", }));
		vbox.append(new TextField({ placeholder: "Username" }));
		vbox.append(new TextField({ placeholder: "Password" }));
