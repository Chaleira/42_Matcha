import { AnchorElement, AvatarPanel, ButtonElement, CardPanel, Component, DivElement, DropDown, H4Element, HBox, ref, Router, SpanElement, TextAreaElement, TextField, VBox } from "typecomposer";
...
export class RegisterPage extends Component {
	...
	private step1(): VBox {
		...
		vbox.append(new TextField({ placeholder: "Username", label: "Username", value: this.user.value.username }));
		vbox.append(new TextField({ placeholder: "Email", label: "Email", value: this.user.value.email }));
		vbox.append(new TextField({ placeholder: "Password", label: "Password", type: "password", value: this.user.value.password }));
		vbox.append(new TextField({ placeholder: "Confirm password", label: "Confirm password", type: "password", value: this.user.value.confirmPassword }));
	...
	private step2(): VBox {
		const vbox = new VBox({ padding: "10px", gap: "15px" });
		vbox.append(new H4Element({ text: "Register", className: "login_header", }));
		vbox.append(new TextField({ placeholder: "First Name", value: this.user.value.firstName }));
		vbox.append(new TextField({ placeholder: "Last Name", value: this.user.value.lastName }));
		vbox.append(new TextField({ type: "date", placeholder: "Date Birth", value: this.user.value.dateBirth }));
	...
	private step4(): VBox {
		...
		vbox.append(new AvatarPanel({ src: this.user.value.avatar }));
